...
import os
os.makedirs("./preds/test/layer1", exist_ok=True)
os.makedirs("./preds/test/layer2", exist_ok=True)
os.makedirs("./preds/cluster/layer1", exist_ok=True)
os.makedirs("./preds/cluster/layer2", exist_ok=True)
os.makedirs("./stats/clustering", exist_ok=True)
...
for round in range(NUM_ROUNDS):
    ...
    def predictUser(i):
        preds1 = []
        start = NOTES_TEST * i
        end = min(start + NOTES_TEST, len(testX))
        if start >= len(testX):
            return -1  # 더 이상 데이터 없으면 무시

        for j in range(LAYER_1_SIZE):
            preds1.append(clfs1[j].predict_proba(testX[NOTES_TEST*i:NOTES_TEST+NOTES_TEST*i]))
        pred1 = np.hstack(preds1).sum(axis=0)
        mtrxAtest.append(pred1)

        preds2 = []
        for j in range(LAYER_2_SIZE):
            preds2.append(clfs2[j].predict_proba(testX[NOTES_TEST*i:NOTES_TEST+NOTES_TEST*i]))
        pred2 = np.hstack(preds2).sum(axis=0)
        mtrxBtest.append(pred2)

        preds1C = []
        for j in range(LAYER_1_SIZE):
            preds1C.append(clfs1[j].predict_proba(clusterX[NOTES_TEST*i:NOTES_CLUSTER+NOTES_TEST*i]))
        pred1C = np.hstack(preds1C).sum(axis=0)
        mtrxAcluster.append(pred1C)

        preds2C = []
        for j in range(LAYER_2_SIZE):
            preds2C.append(clfs2[j].predict_proba(clusterX[NOTES_TEST*i:NOTES_CLUSTER+NOTES_TEST*i]))
